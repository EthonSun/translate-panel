...
- 可选：`sdcv` + StarDict 词典（放 `~/.stardict/dic/`）。装了就支持**单词/短语查词典**
  （英≤3 词、中≤6 字、精确匹配命中才显示，否则回退 DeepSeek；整句直接 DeepSeek）。
  词典可从 <http://download.huzheng.org/> 下载，如牛津英汉 `stardict-oxford-gb`、
  朗道汉英 `stardict-langdao-ce-gb`。词典释义的整理逻辑按牛津英汉格式做，换别的词典基本原样显示。
